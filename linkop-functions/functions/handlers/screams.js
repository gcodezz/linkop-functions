...
const { validateScream } = require('../utils/validators')
...
exports.postOneScream = (req, res) => {
    ...
    const { errors, valid } = validateScream(newScream.body)
...
exports.likeScream = (req, res) => {
    const likeDocument = 
        ...
        screamDocument.get()
            ...
            .then(data => {
                if (data.empty) {
                    return db.collection('screamsLikes').add({
                        screamId: req.params.screamId,
                        userHandle: req.user.handle
                    })
                    .then(() => {
        ...
        screamDocument.get()
            ...
            .then(data => {
                if (data.empty) {
                    ...
                    return db
                        .doc(`/screamsLikes/${data.docs[0].id}`)
                        .delete()
                            .then(() => {
                                screamData.likeCount--
                                return screamDocument.update({
